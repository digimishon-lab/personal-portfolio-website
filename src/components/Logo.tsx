...
export default function Logo({ 
  darkMode = true, 
  className = "",
  useImage = false, // Default to false so that the high-fidelity pure-vector SVG version is rendered
  imageSrc = "/logo.png" // Path to original logo if needed
}: LogoProps) {
  return (
    <div className={`flex items-center gap-3 select-none ${className}`}>
      {useImage ? (
        /* =========================================================================
           ...
           2. Set useImage={true} when calling this component.
           ========================================================================= */
        <img
          src={imageSrc}
        ...
        /* =========================================================================
           OPTION B: HIGH-FIDELITY INLINE SVG MONOGRAM & VECTOR TYPOGRAPHY
           Provides crisp, lossless rendering at any size or pixel density, exactly matching the design!
           ========================================================================= */
        <div className="flex items-center gap-3">
          ...
          {/* Thin vertical separator line matching the original logo */}
          <div className="w-[1.5px] h-9 bg-white/25 self-center rounded-full" />

          {/* Brand Stacked Typography */}
          <div className="flex flex-col justify-center select-none pl-0.5">
            <span className={`font-display font-black text-[13px] tracking-[0.16em] leading-none uppercase ${darkMode ? "text-white" : "text-slate-900"}`}>
              DIGITAL
            </span>
            <span className="font-display font-black text-[13px] tracking-[0.16em] leading-none uppercase mt-0.5 text-transparent bg-clip-text bg-gradient-to-r from-[#2563FF] to-[#7C3AED]">
              MISHON
            </span>
            <div className={`text-[4.8px] font-mono font-black tracking-[0.14em] uppercase leading-none mt-1 flex items-center gap-1 ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
              <span>DESIGN</span>
              <span className="text-[#2563FF] text-[6px]">•</span>
              <span>AUTOMATE</span>
              <span className="text-[#2563FF] text-[6px]">•</span>
              <span>GROW</span>
            </div>
